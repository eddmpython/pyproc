...
- 모든 커밋 전 `npm test` green. 게시하려는 트리에 대해 `.githooks/pre-push`가 같은 게이트를 다시
  돌린다. 커밋 메시지의 검증 줄은 주장이고 푸시 게이트가 확인이다. RED 트리는 `origin/main`에
  닿지 못하며 `--no-verify`는 우회로가 아니다.
...
- **커밋 메시지는 라벨이 아니라 기록이다**(기계 강제). 릴리즈 노트, 사고 조사, 회귀 추적이 전부
  ...
  무엇을 어떻게 바꿨는지(파일·심볼 수준).
  왜 필요했는지(문제·근거. 되돌리려는 사람이 판단할 수 있게).
  검증: 어느 게이트가 green인지. 신설 게이트면 음성 시험 결과까지.
  ```

  - **제목**: `분류: 요약` 형식, 72자 이내, 마침표 없음, 한국어.
  - **본문 필수**: 빈 줄로 분리, 최소 2줄, 줄당 100자 이내.
  - **검증 줄 필수.** 무엇으로 확인했는지 없는 변경은 기록이 아니라 주장이다.
