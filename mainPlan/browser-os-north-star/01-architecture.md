...
| 부팅/영속 | `bootEnv`, `freeze`, `MachineJournal`, `MachineJournal.pack/prune/autoPack`, `KernelElection`, `SharedKernel` | SharedKernel과 hibernate/resume 결합, 부활 후 fd 재개설 정책 |
...
3. 부활 후 fd, socket, DB connection 같은 프로세스 자원은 어떤 init 계약으로 재개설할 것인가.
