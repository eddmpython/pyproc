...
| 메모리 | `ReactiveController`, checkpoint tree, `Session` | 500MB 이상 힙에서 checkpoint/restore/fork/journal 비용 봉투 |
...
| 보호 | `MachineJail`, trust flag, CSP, `.pymachine` WebCrypto signature, 부트 자산 SRI v2(`pyodide.js` + fetch core + pyproc worker graph preflight), 실행 자산 manifest | 공개키 배포, Service Worker 등록 자산과 Pyodide 내부 import 모듈 봉인, 권한 UI는 소비 제품 계약으로 분리 |
| 부팅/영속 | `bootEnv`, `freeze`, `MachineJournal`, `KernelElection`, `SharedKernel` | SharedKernel과 hibernate/resume 결합, 저널 pack/prune |
...
3. Journal pack/prune 없이는 장수 머신이 어느 시점에 느려지는가.
