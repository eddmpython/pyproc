...
| 보호·격리 | 5 | 7 | worker 주소공간 격리, `.pymachine` trust gate + WebCrypto signature, 공개키 fingerprint API, trust/permission UI 계약, 부트 자산 SRI v2, `registerPyProcServiceWorker`, SW `coreIntegrity`, `MachineJail` 협조 티어 + CSP connect-src 집행. 근거: [machineJail.js](../../src/capabilities/machineJail.js), [session.js](../../src/capabilities/session.js), [trustPermissions](../../docs/consuming/trustPermissions.md), [runtime.js](../../src/runtime/runtime.js), [assets.js](../../src/runtime/assets.js), [pyprocSw.js](../../src/capabilities/pyprocSw.js), [jailProbe](../../tests/attempts/pythonMachine/README.md) | same-origin parent 측면통로, 외부 제품 trust/permission UI gate 없음, 힙 평문 비밀 경고 필요 |
...
| 영속·크래시 내성 | 7 | 8 | MachineJournal WAL, recover h0 대조, `pack()`/`prune()` live blob compaction, `autoPack` opt-in 정책, `Init.resume` 부활 후 자원 재개설, resume catalog, Machine demo의 SQLite connection 재개설, KernelElection failover, Session revival, `/home` 포함 signed `.pymachine`. 512MB journal commit/recover는 2-3초대, pack은 1.1초대로 진입했다. 근거: [machineJournal.js](../../src/capabilities/machineJournal.js), [session.js](../../src/capabilities/session.js), [init.js](../../src/capabilities/init.js), [resumeCatalog](../../docs/consuming/resumeCatalog.md), [machine demo](../../examples/machine.html), [kernelElectionProbe](../../tests/attempts/pythonMachine/README.md), [journalPackProbe](../../tests/attempts/pythonMachine/journalPackProbe.html), [resumeHookProbe](../../tests/attempts/pythonMachine/resumeHookProbe.html), [largeHeapEnvelope](../../tests/attempts/largeHeapEnvelope/README.md) | 외부 제품 trust/permission UI와 resume.py gate 필요 |
...
보류 문장을 열려면 외부 제품 소비 배선을 machine image 또는 VirtualOrigin 축까지 넓히고, trust/permission UI를 외부 제품 gate로 닫아야 한다. journal pack/prune/autoPack과 `Init.resume`은 구조와 운영 기준을 닫았고, VirtualOrigin 경계 lab은 쿠키/WS/스트리밍 벽을 공개 계약으로 닫았다. `resume.py` 자원 정책과 공개키/권한 UI 계약은 Machine demo와 카탈로그까지 닫혔지만, codaro/dartlab/xlpod 같은 외부 제품 gate에는 아직 붙지 않았다.
