...
단, 무수식 "로컬 OS" 또는 "리눅스급 웹 OS"는 아직 아니다. 2026-07-14에서 2026-07-15 사이에 512MB checkpoint/session/fork/journal 비용은 [05-large-heap-envelope.md](05-large-heap-envelope.md)로 실측됐고 journal recover 병목은 24.8s에서 2.3-2.5s로 줄었다. MachineJournal pack/prune은 장기 loose blob 누적 구조를 닫았고 512MB pack은 1.1s로 들어왔다. `.pymachine` signature, 부트 자산 SRI v2, Service Worker 등록 자산 봉인, codaro의 첫 제품 소비 증거도 들어갔지만, 공개키 배포 UI와 권한 UI는 남아 있다.
...
보류 문장을 열려면 제품 소비 배선을 machine image 또는 VirtualOrigin 축까지 넓히고 공개키·권한 UI 계약을 닫아야 한다. journal pack/prune은 구조와 512MB 수치를 닫았지만, 자동 pack 정책 기준은 제품 장수 운영 기준으로 남긴다.
