...
- **D2 관문 - non-Pyodide 실부팅** (attempts enginePort): **1번 관문 졸업(2026-07-12, GREEN 6/6)**. WLR CPython 3.12(WASI) + vendored browser_wasi_shim(의존성 0)로 워커 부팅 253ms + 계약 코어 2축(선형 메모리 + 결정적 부팅) 실측. "떼기"가 개념에서 실측이 됐다. 남은 질문: 값 프로토콜(FFI 우회)/반복 실행/reactive 이식 -> 승격 시 `boot({engine})` 옵션 + WasiEngine. 정본: [enginePort](../../tests/attempts/enginePort/README.md).
