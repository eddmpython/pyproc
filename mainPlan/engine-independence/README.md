...
상태: PRD (2026-07-12, 전문 에이전트 2종 연구 종합: 생태계 리서치 + 코드 결합 감사). **착수는 소유자 승인 대기.**

## 질문과 답

소유자 질문: "Pyodide 의존성을 제거하고 독립적으로, 지금 모든 게 가능한 방법 - 아니면 더 훌륭한 방법은?"
...
- **P1 코드 독립 - EngineContract seam** (주 단위): 감사의 계약안 채택. `src/runtime/engines/pyodideEngine.js`(신규 ~250 LOC, 대부분 기존 코드 이동)로 엔진 접점을 격리하고 Runtime/능력은 계약만 호출. 파이썬측은 어댑터가 `pyprocFfi` shim 주입(runSync 등). 게이트: **동작 무변경** = 구조 + 브라우저 게이트가 수정 없이 GREEN.
