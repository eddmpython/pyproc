...
**종결 4건**: `externalS1`(지식이 benchmarking.md에 승격돼 있었다)과 `branchFleet`(주석의 경로 인용을
...
폴더는 증인이 될 수 없으므로 공유의 진짜 계약인 재구현 금지를 직접 무는 법으로 바꿨다).

**결론: 나머지 10개는 지금 지울 수 없다.** 이유가 폴더마다 다르지 않다:
