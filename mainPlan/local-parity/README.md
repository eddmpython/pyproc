...
- **아웃바운드 소켓**: **진짜 파이썬 socket이 진짜 TCP를 연다** - Pyodide의 `socket.socket()`을 얇은 WS->TCP 릴레이에 심해 Python `connect/send/recv`가 example.com:80에서 `HTTP/1.1 200`을 받는다(블로킹 recv = SAB/Atomics). [socketBridge GREEN 3/3 + 2/2 + 파이썬 2/2](../../tests/attempts/socketBridge/README.md). `requests`/`urllib3`가 같은 socket API라 따라온다. 남은 것 = src 능력 승격(`enableSocketBridge`) + Wisp 멀티플렉싱.
