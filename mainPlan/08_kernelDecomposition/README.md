...
남은 것: `KernelElection`의 3파일 분할. 이 항목의 **정확성 목표는 위에서 이미 달성**됐다
(다음 회귀의 진원지로 지목된 것이 정책 이중화였고 그것이 게이트로 닫혔다). 남은 이득은
가독성이고, 676줄을 세 파일로 가르는 작업은 28개 상태 필드가 `status()`에서 다시 합쳐지는
구조라 한 번에 끝나지 않는다. 착수할 때는 membership(presence + heartbeat) 추출부터 한다:
`_participants`가 13곳에서 쓰이지만 전부 note/expire/remove/entries로 환원된다.
