...
| adapter | 첫 근거 | 역할 |
|---|---|---|
| pyproc | `openPersistentMachine`, `Session`, `MachineJournal` | 첫 guest OS, heap + `/home/web` + failover |
| WASI | `bootWasi`, `WasiSession` | 두 번째 엔진으로 lifecycle contract의 엔진 독립성 조기 증명 |
| x86 | 외부 주입형 v86 adapter 후보 | 실제 Linux/ReactOS 계열 부팅으로 Dual-Boot 주장 검증 |
...
├─ pyprocGuestProbe.html
├─ wasiGuestProbe.html
└─ dualBootProbe.html
```

두 엔진과 한 Linux guest가 계약을 통과한 뒤 다음을 결정한다.
