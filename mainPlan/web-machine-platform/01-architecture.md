...
| adapter | 첫 근거 | 역할 |
|---|---|---|
| pyproc | `openPersistentMachine`, `Session`, `MachineJournal`, [dualEngineProbe](../../tests/attempts/webMachine/dualEngineProbe.html) GREEN 13/13 x3 | 첫 guest OS, `portable` snapshot, heap + `/home/web` + failover |
| WASI | `bootWasi`, `WasiSession`, [dualEngineProbe](../../tests/attempts/webMachine/dualEngineProbe.html) GREEN 13/13 x3 | 두 번째 엔진, `session` snapshot, lifecycle contract의 엔진 독립성 증명 |
| x86 | 외부 주입형 v86 0.5.424, [linuxGuestProbe](../../tests/attempts/webMachine/linuxGuestProbe.html) GREEN 8/8 x3, [dualBootProbe](../../tests/attempts/webMachine/dualBootProbe.html) GREEN 8/8 x3 | Buildroot Linux 6.8.12 i686 `portable` state, pyproc + Linux 실제 Dual-Boot 증명 |
...
두 엔진과 한 Linux guest는 계약을 통과했다. 공통 block/network/display와 영속 commit을 검증하며 다음을 결정한다.

- host가 pyproc과 릴리즈 주기를 공유해도 되는가.
- `src/machineHost/` 새 레이어가 맞는가.
- 독립 package/repository가 의존 방향을 더 깨끗하게 만드는가.

결정 전에는 pyproc 공개 export를 늘리지 않는다.
