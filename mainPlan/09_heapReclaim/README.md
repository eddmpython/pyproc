...
닫힌 것: 스냅샷 SAB 회수(03에서 당겨왔다). `samePage` 확정 비교의 워드화(fork 수확 실측
43.6ms 대에서 11.2ms로. property 시험이 바이트 비교와의 동치를 매 실행 대조한다).
`collectDelta`의 이중 복사 제거. 죽은 프로세스 엔트리 상한과 `createRpcPort.dispose()`.
`MachineHandle._history` 상한.

남은 것 셋과 그 이유:

- **워커 cp0의 해시 강등**: 이 계획의 제품 경계가 금지한다("성능 개선을 위해 정확성 강도를
  낮추지 않는다"). fork 수확의 판정 근거를 바이트 동일성에서 64비트 해시 충돌 확률로 내리는
  일이고, fuzz는 충돌 부재를 증명할 수 없다. 워드화가 같은 경로의 비용을 이미 1/4로 낮췄으므로
  남은 이득은 메모리(2NH -> (N+1)H)뿐이다. 채택하려면 reactive와 fork의 판정 강도를 통일하는
  결정이 먼저이고, 그것은 이 캠페인의 범위가 아니라 제품 결정이다.
- **스냅샷 생성의 워커 이관**: 선결 probe가 있다(메인에서 만든 스냅샷과 워커에서 만든 스냅샷의
  바이트 동일성). `worker.js` 헤더가 메인 커널과 워커 커널의 리플레이는 바이트가 다르다고
  기록하고 있어 실측 없이 판단하면 안 되는 자리다.
- **fork 델타의 SAB 직접 수확**: 커널측 memcpy 1회를 없앤다. 위험은 낮지만 harvest 프로토콜을
  2단계로 바꿔야 해서 워커 계약 변경이 함께 온다.
