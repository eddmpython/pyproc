...
- 남은 대형 파일은 `v86GuestAdapter`(437줄)이고 이음매는 `_createEmulator`(약 98줄)다: 매니페스트와
  장치로 V86 인스턴스를 조립하는 관심사가 수명주기 동사들과 한 파일에 산다.
...
1. **v86GuestAdapter 분해**: `_createEmulator`(약 98줄)를 엔진 조립 모듈로 분리한다. 어댑터에는
   수명주기 동사만 남긴다.
