...
## 2026-08-01 (5): 러너 분해 1차

- `tests/support/` 신설(재료만 살고 판정은 절이 소유한다는 규칙을 README가 말한다). 첫 입주는
  시드 고정 난수원이다: property 절 둘이 각자 mulberry32를 들고 있었고, 사본이 둘이면
  "시드 7의 12번째 반복"이 절마다 다른 값을 가리켜 실패 보고가 재현 지시가 아니게 된다.
- property 4절(해시 soundness, 봉투·이미지 경계, reactive 나무, election 프로토콜)의 **본문**을
  각자 모듈로 옮겼다. run.mjs 4040 -> 3570줄. 판정 이름과 개수는 그대로이고 check/checkAsync는
  러너가 주입한다. contracts 스위트로 옮기지 않은 이유는 형태다: 그쪽은 단일 assert가 던지는
  모양이라 절을 옮기면 검사 단위가 사라지고 게이트 층 하한이 셀 것이 없어진다.
- 남은 것은 `[구조]` 절(715줄)이다. 이 절은 공유 상수를 낀 평면 시퀀스라 블록 추출이 안 되고,
  4분할하면 섹션 이름이 바뀌어 하한 키가 함께 움직인다. 분해 단위는 "상수와 그것을 쓰는 검사"
  묶음이어야 하고, 그 묶음을 먼저 식별하는 것이 다음 단계다.

## 2026-08-01 (6): 러너 분해 완료와 대형 파일 1차

- **[구조] 절을 세 묶음으로 나눴다**(Web Computer 제품 5, 문서 수명주기 4, Web Machine 층 16).
  나눈 기준은 의존이다: 각 묶음은 자기 상수만 쓰고 러너가 주는 것은 공용 헬퍼와 check뿐이다.
  두 상수(machineRoot, machinePureFiles/machineFileRank)는 두 묶음이 함께 쓰므로 러너에 남겼다.
  절 이름은 하나로 유지했다: 검사 이름과 개수가 그대로라 게이트 층 하한이 한 줄도 안 움직인다.
  `tests/run.mjs` 4040 -> 3115줄.
- **로케일 비교자 금지를 tests/apps/scripts로 넓혔다**(112 -> 188검사, 위반 0). 게이트와 도구가
  로케일 정렬을 쓰면 "이 커밋에서 통과했다"가 기계마다 다른 말이 된다. 음성 시험으로 이빨 확인.
- **indexedDbMachineStore 370 -> 341줄**: IndexedDB 콜백 API를 promise로 바꾸는 원시 어댑터를
  `indexedDbPrimitives.js`로 분리했다. 층위가 이유다: 그 넷은 브라우저 API가 콜백이라는 사실만
  알고 owner/blob/generation/HEAD가 무엇인지 모른다.
- **v86GuestAdapter에서 준비 판정 축을 뺐다**(`v86Readiness.js`). 어댑터 상태를 통째로 만지지
  않는 것이 축이 맞다는 증거다: 필요한 것은 선언과 포트 둘뿐이라 인자로 받는다. x86 자산 레인으로
  실제 Linux guest를 부팅해 검증했다(GREEN 16/16 probe).
- `_createEmulator`(98줄)는 쪼개지 않기로 판정했다. 실측: 그 안에서 어댑터 상태를 47회 만지고
  대상 필드가 23개다(포트 여덟 개를 만들어 대입하는 것이 그 절반이다). 순수 함수로 빼면 인자
