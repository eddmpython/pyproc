...
> ✅ 완료 (2026-07-13): P1 seam + D2 non-Pyodide 완전 실증 + WASI src 승격(bootWasi/WasiSession) + brettcannon 3.14.6 이전 + P3 engine-watch 자동성장 + P0 자가 호스팅(유일 실시간 CDN 리스크 제거) + P2 스냅샷 사전 제조 벽 좌표 실측. 사다리의 실측 rung 전부 닫힘. P4(조건부 fork 보험)만 미발동(발동 조건 미충족 = 대기 보험). 재발동 시 새 이니셔티브.

상태: **사다리 실증 완결 (2026-07-13).** P1 seam + D2 관문(non-Pyodide 완전 실증) + WASI src 승격(bootWasi/WasiSession) + 죽은 WLR 3.12 -> 살아있는 brettcannon 3.14.6 이전 + P3 engine-watch 자동성장 + **P0 자가 호스팅(유일 실시간 CDN 리스크 제거)** + **P2 스냅샷 사전 제조 벽 좌표 실측**까지 전부 실행했다. 상세 실행 기록은 [web-python-runtime 원장](../web-python-runtime/03-progress-ledger.md)(2026-07-13 항목들)이 정본. **P4(조건부 fork 보험)만 미발동 상태로 남는다**(발동 조건 = 스냅샷-with-packages가 제품 차단 + 업스트림 부재, 둘 다 참일 때만. 오늘 발동 사유 없음). 사다리의 실측 rung은 전부 닫혔다. 아래 사다리에 완료/열림 표시.
...
- ~~**P0 유통 독립 - 자가 호스팅**~~ **완료(2026-07-13)**: `scripts/fetchEngine.mjs`(`npm run fetch:engine`)가 GitHub Releases 전체 배포판(426MB, 전 패키지 wheel 포함)을 `vendor/pyodide/`로 준비(멱등, 레포 자산 0 = gitignore). 게이트/probe가 `?indexURL=` 오버라이드를 받아 같은 검사를 자가 경로로 돈다(`PYPROC_INDEX_URL=/vendor/pyodide/ npm run test:browser`). **실측: 자가 경로에서 게이트 전 검사 39/39 GREEN**(부팅 2760ms, 휠 설치·freeze 락 354패키지까지 CDN 0) + offlineBoot 4/4 + swOffline 3/3 재실측 GREEN. docs/consuming/contract.md 갱신. 핀 정합(fetchEngine == DEFAULT_INDEX)은 tests/run.mjs가 기계 검사. 유일한 실시간 리스크(CDN)가 제거됐다.
...
- ~~**D2 관문 - non-Pyodide 실부팅**~~ **완전 실증·src 승격 완료(2026-07-13)**: 1번 관문(부팅 253ms + 선형메모리 + 결정적 부팅, GREEN 6/6) 이후 남은 질문을 전부 뚫었다 - 값 프로토콜(FFI 우회 JSON 다리) + 반복 실행 + **완전 시간여행**(트랩 근원 규명 후 할당 불변 경계 `readinto` + 파티션 복원으로 해결) + 패키지(`installWheel` = 브라우저판 pip) + 정적 C확장 확인 + **죽은 WLR 3.12 -> 살아있는 brettcannon 3.14.6 이전**까지. 승격 표면 = `bootWasi`/`WasiSession`(별도 async 표면 = sync Runtime/PyProc 무영향). 정본: [enginePort](../../../tests/attempts/enginePort/README.md) + [원장](../web-python-runtime/03-progress-ledger.md) 2026-07-13.
- ~~**P2 기능 독립 - 스냅샷 사전 제조 probe**~~ **실측 완료(2026-07-13)**: [prefabSnapshotProbe](../../../tests/attempts/envManager/prefabSnapshotProbe.html) GREEN 8/8. **벽 좌표 정밀화(소스 + 실측)**: 직렬화기가 부팅 시 확정된 hiwire 슬롯 0..6(정확히 7개)을 `checkEntry`로 먼저 검사하고, 슬롯 6은 매번 새 `{}`라 "빈 객체" 구조 검사만 성립한다. `loadPackage`가 그 객체에 채널 dict(`{"numpy":"default channel"}`)를 남기는 순간 `Unexpected hiwire entry at index 6` = 벽. serializer 인자는 검사 이후에 개입하므로 우회 불가(스톡 knob 0). **그러나 벽은 loadPackage 기계지 dlopen이 아니다**: 순수 휠을 `FS.writeFile` + `zipfile.extractall(purelib)` + import로 주입하면(loadPackage 우회) slot diff 0, extra 1로 **채취 성공** + 웜 부팅에서 재설치 0 생존(six 1.17.0, 채취 4ms, 웜 109ms). Cloudflare workerd 패턴(자체 FS 마운트 + import 후 캡처)의 순수 휠 부분이 스톡에서 재현됨을 확증. 정직 경계: 미import 서브모듈·데이터 파일은 스냅샷 밖(웜에서 재주입 필요), C확장 휠은 여전히 dlopen 상태 재생이 없어 벽. **승격 보류(정직)**: 순수 휠은 이미 bootEnv(bare + OPFS 휠 + import)가 3.49배를 내고 순수 휠 import는 짧아 prefab의 한계 이득이 작다. 덕지덕지 금지 원칙상 probe로 벽 좌표를 확정하고 upstream 트랙을 관찰하는 것이 옳은 착지. 상세: [envManager 캠페인](../../../tests/attempts/envManager/README.md).
- **P3 업스트림 워치/기여**: (a) Pyodide 스냅샷 벽 트랙 - #5195(벽 본체), #5971(공식 해결 트랙), 스냅샷 안정화 언질(유지보수자, 2025-12-25). fork보다 싼 길 = 이 트랙 기여로 착지를 앞당기기(열림). (b) **WASI 엔진 자동성장 - 실현(2026-07-13)**: `.github/workflows/engine-watch.yml`이 brettcannon 릴리즈를 감지해 핀과 다르면 후보 Issue(봇 커밋 없음, 승격은 릴리즈 결정). 코어가 엔진 무관이라 "새 바이너리 + 결정적 게이트 통과 = 성장"이 성립. = "파이썬과 함께 자란다"의 배관.
