...
| ~~P6~~ | ~~**machineJail (권한 매니페스트)**~~ **완료(2026-07-13)** | 머신 헤더에 `permissions{net, clipboard, home, workers}`. 집행 2단: 협조(우리 초크포인트) + **감옥**(CSP `connect-src` = 브라우저 벽) | trust:true 이진 게이트가 스코프 승인으로 진화. 파이썬 레벨 검사는 `import js`로 우회 가능함을 인정하고 **브라우저의 벽**을 빌리는 정직한 설계 | **승격: `MachineJail`.** jailProbe GREEN 8/8: 협조 티어(pyprocJail.net/home = 비허용 PermissionError), **CSP connect-src 'self'가 example.com fetch 차단**(감옥 코드 우회 무력 = 브라우저 벽) + self 허용, 감옥 CSP에서 pyodide 부팅(6*7=42) + **오버헤드 0.93x**(CSP는 헤더). 정직 경계 실측: same-origin 감옥은 window.parent 도달(네트워크 벽만) vs opaque origin 감옥은 부모 차단(완전 격리지만 SAB 포기). 감옥은 자가 호스팅 엔진(connect-src 'self')을 전제 = P0과 짝 |
...
**경계(2026-07-13 확정)**: 아래 6종은 전부 **이미 승격된 프리미티브 위의 소비 제품/examples UI 표면**이지 새 src 능력이 아니다. 근거 프리미티브가 전부 서 있다: /dev/display = `DeviceFs` framebuffer/flush(P7 승격), Time Rail = `reactive.tree()`(체크포인트 나무, 승격), addressable blocks = 같은 나무 + 출력 상태, session cast = `exportImage`/`openMachine`(승격), `&` UI = `JobControl`(P3 승격), ghost history = `rlcompleter`(표준 라이브러리) + `Terminal`. "제품 UI는 소비 제품 몫" 원칙(이 문서 원칙 절)이라 pyproc src 로드맵이 아니라 소비자 표면 후보다. 신규 캠페인 불필요.
...
이 8종은 지속 결정 가드레일로 승격됐다: **정본 = [docs/product/vision.md](../../../docs/product/vision.md)의 "안 만드는 것"**. 아래는 각 항목의 전체 논증(승격본은 항목당 1줄 요약)이다.
