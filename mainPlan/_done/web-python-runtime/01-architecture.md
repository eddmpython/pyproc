...
이 표는 지속 운영 문서로 승격됐다: **정본 = [docs/operations/contractReality.md](../../../docs/operations/contractReality.md)** (열린 부채·상시 재검증·트레이드오프·프론티어). 이니셔티브는 완료 시 `_done`으로 이관되므로 상시 추적 원장은 docs에 산다. 아래는 이 이니셔티브 진행 중 닫힌 간극의 이력 기록이다.

| 항목 | 계약 | 실제 | 상태 |
|---|---|---|---|
| restoreLive 실행 경계 | 경계 준수 시 즉시(재해싱 0), 위반 시에도 조용한 오염 없음 | **기계 강제(2026-07-11)**: Runtime.execSeq(상태 변이 카운터)로 위반을 O(1) 감지해 자동 재해시 승격 | 해소 (외부 리뷰 지적 반영) |
...
| syscallBridge | input/HTTP/subprocess를 실제로 빌린다 | v1 실배선: input(동기 + JSPI `run_sync`), urllib(동기 XHR), subprocess(자식 워커). 저수준 socket은 이후 `SocketBridge`로 승격 | 해소 (attempts/syscallBridge 졸업, 2026-07-11) |
| PyProc 오류 경로 | 부팅·행·죽음이 유한 시간에 귀결 | 부팅 실패 reject + `map(.., {taskTimeoutMs})` 행 수렴 + kill/스냅샷 respawn + 협조적 취소(SIGINT) | 해소 (attempts/processLifecycle 졸업, 2026-07-11) |
| README 표면 동기화 | 공개 표면 = README 표 | `tests/run.mjs`가 index.js의 모든 export가 양쪽 README에 등장하는지 기계 검사(2026-07-12) | 해소 (가드 상시) |
| restore()의 힙 성장 처리 | 두 복원 경로의 성장 처리 동등 | 비대칭은 존재하나 **실해 없음 실측**(growthRestoreProbe, 2026-07-11) | 해소 (실측으로 종결) |

열린 부채·상시 재검증 항목(암묵 FFI/fetch 가정, %undo 메모리, Pyodide 스냅샷 API, 전역 스텁, 자가 호스팅 핀, 감옥 부모 격리, memcpy 1회)은 [docs/operations/contractReality.md](../../../docs/operations/contractReality.md)가 정본으로 계속 추적한다.
