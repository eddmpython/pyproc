...
pyproc 공개 API에 x86 또는 Linux 특수 로직을 넣지 않는다. attempts 캠페인에서 공통 장치와 durable commit까지 검증했고, [독립 package 구조](04-clean-architecture-and-code-rules.md)와 [정식 검증 트리](../../../tests/webMachine/)로 승격했다.
...
pixel 화면은 text display에 mode flag를 덧붙이지 않는다. `rgba-frame` display가 bounded RGBA8888 region을
working frame에 쓰고 revision 단위로 present하며, canvas frame source는 실제로 바뀐 dirty region만 읽는다.
...
달라진 순간 outcome unknown이며 자동 replay하지 않는다.
