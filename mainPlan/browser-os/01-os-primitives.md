...
|---|---|---|---|---|
| P1 | **machineJournal (WAL + 페이지 CAS)** | 문장 경계마다 변경 페이지를 content-addressed로 OPFS에 append + commit 레코드(부모 포인터). 부팅 = 리플레이 + 최신 commit | **브라우저를 강제 종료해도 마지막 문장으로 부활**한다(로컬 REPL도 못 준다). 페이지 해시가 이미 있어 CAS 키가 공짜이고, 분기(나무)와 같은 기계장치 | 문장당 델타 분포 실측, append p50 < 15ms, 크래시 후 힙 다이제스트 일치, CAS dedupe 후 총량 < 원시의 30% |
