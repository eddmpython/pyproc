...
1. **forkLive = 진짜 fork(2)** - `PyProc({replay})` + `fork(src, dst)`. 살아있는 프로세스의 변수·배열·계산 결과가 자식으로 실린다(델타 10.3MB 수확 43.6ms, 적용 1.4ms, 주소공간 독립).
...
|---|---|---|---|---|
| P1 | **machineJournal (WAL + 페이지 CAS)** | 문장 경계마다 변경 페이지를 content-addressed로 OPFS에 append + commit 레코드(부모 포인터). 부팅 = 리플레이 + 최신 commit | **브라우저를 강제 종료해도 마지막 문장으로 부활**한다(로컬 REPL도 못 준다). 페이지 해시가 이미 있어 CAS 키가 공짜이고, 분기(나무)와 같은 기계장치 | **개념 졸업(journalProbe 5/5, 2026-07-12)**: 부활·dedupe 성립. **발견**: 문장당 churn 중앙값 128p(8MB)/약 1s - GC·할당자 바닥이 높아 naive 문장단위 WAL은 무겁다. **승격 전 필수**: (a) idle-batch(유휴/N문장마다 커밋) 또는 (b) churn 바닥 조사(왜 tiny 문장이 128p를 더럽히나). append는 async(비차단)라 체감은 별개 - 재실측 대상 |
