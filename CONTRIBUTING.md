...
pyproc is a reusable browser Python runtime built around a worker-owned CPython WASI kernel, verified assets, checkpoints, and independent processes. Thanks for your interest. This document is the contract for participating in the repository.

## License and contribution terms

pyproc is licensed under the [Mozilla Public License 2.0](LICENSE). By submitting a contribution you agree it is provided under the same license: under MPL-2.0 a contributor grants the copyright and patent licenses for their contribution by contributing it (Section 2.1), so inbound = outbound holds and no separate CLA is required. If you cannot agree to that, do not submit code.
...
2. **Graduated learnings become maintained contracts** in `skills/`, automated evidence in `tests/`, and reviewable decisions in git history. Temporary planning archives are not kept in the repository.
...
Operating details live in [skills](skills/start-pyproc/SKILL.md).
...
Browser validation: open `http://localhost:8788/examples/basic.html` and `processOs.html` in Chrome/Edge. The page must report `crossOriginIsolated === true`. Real verification of this WASM runtime only happens in a browser; see [skills/verify-pyproc/references/testing.md](skills/verify-pyproc/references/testing.md).
...
- Version stays on the `0.0.x` line; only releases bump it, and the tag must match `package.json`. See [skills/ship-pyproc/references/release.md](skills/ship-pyproc/references/release.md).
