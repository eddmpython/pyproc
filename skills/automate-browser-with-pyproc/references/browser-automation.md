...
```sh
npm install pyproc@0.0.33 --save-exact
...
since a prefetched or prerendered page would be shown from a response fetched outside request interception. Each page
is served again with its connection policy, so the browser counts it as coming from the public internet: its frames
