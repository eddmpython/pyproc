...
| Native Linux CPython image is built by a separate profile | `createWebComputer().linuxPython`이 `linuxOs` serial로 `python3`와 `python -m pip`를 친다. 기본 `boot()`는 WASI다. `python` profile은 CPython 3.12.13과 pip 25.2를 고정한다 | 공개 catalog에 `buildroot-pyproc-python-i686.bin`이 실리려면 독립 A/B build와 legal-info가 먼저 끝나야 한다. slim linux image는 그대로 CPython이 없다 |
