...
| Proof-carrying browser action convergence | 문서 교체 뒤 원래 typed focus가 유일한 경우에만 한 번 재발급하지만 같은 문서의 stale target은 복구하지 않고, 모호함과 가림 거절에는 공통 시간 상한 및 effect 영수증이 없으며, FrameSpace는 가려진 locator를 실행 직전에 막지 않는다 | 후보 2개, 재관찰 1회, effect retry 0회, 첫 effect 전 30초 상한을 모든 provider의 성공과 안전 거절에 같은 영수증으로 증명한다 |
