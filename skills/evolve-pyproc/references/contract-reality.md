...
| Native Linux CPython image is built by a separate profile | `createWebComputer().linuxPython`이 `linuxOs` serial로 `python3`와 `python -m pip`를 친다. 기본 `boot()`는 WASI 3.14.6이다. guest는 CPython 3.12.13이다. inspect가 그 분리를 말한다. receipt `stdout`은 serial 원문이다. 오프라인 없는 휠은 guest pip가 거절한다 | 공개 catalog에 `buildroot-pyproc-python-i686.bin`을 올리려면 검증 artifact와 complete legal-info로 release를 조립해야 한다. slim linux image는 그대로 CPython이 없다 |
