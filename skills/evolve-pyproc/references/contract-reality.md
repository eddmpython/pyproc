...
| Package reach plan predates the owned WASI engine | `packageReach`의 `pyemscripten wheel` 문구는 제거된 Emscripten engine을 전제한다. 현재 기본은 `wasm32-wasip1` CPython이고 resolver는 `py3-none-any`만 받으며 native profile은 lock identity일 뿐 제공 package를 선언하지 않는다 | installed owned engine에서 pure wheel 대조군과 native wheel 첫 실패를 재측정하고, source-built native package catalog와 engine-bound install receipt를 먼저 만든 뒤 scientific SIMD profile로 넓힌다 |
