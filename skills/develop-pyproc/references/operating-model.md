...
| 지속 지식 | [`skills/`](../../start-pyproc/SKILL.md) | 제품 방향, 패키지 계약, 레퍼런스, 반복되는 운영 정책 | "다음 릴리즈에서도 계속 유효한가?" |
...
아이디어
  └─ tests/attempts/<카테고리>/     개념증명. 브라우저 실측으로 졸업 게이트 통과까지
       └─ skills/의 해당 owner      지속될 제품·소비·운영 결정을 갱신
...
- **지속되는 결정만 skill owner로 승격한다.** 제품 방향은 `skills/understand-pyproc/references/vision.md`, 실제와 계약의 열린 차이는 `skills/evolve-pyproc/references/contract-reality.md`, 소비 표면은 intent별 usage skill, 함수 계약은 `skills/reference-pyproc-api/`가 소유한다. 일회성 작업 계획을 새 영구 트리로 만들지 않는다.
...
- **호출자 실측은 재현 가능한 입력으로 승격한다.** 다른 제품이 pyproc의 공개 브라우저 조작 또는 시각
  ...
  [소비 저장소 피드백 계약](../../evolve-pyproc/references/consumer-feedback.md)에 따라 과정과 증거를 비식별화해 기록한다. 지속
  템플릿은 `skills/evolve-pyproc/`가 소유하고, 실제 미완료 건만 직렬 `mainPlan/` 이니셔티브가 된다.
...
4. **바닥부터 다진다.** 검증 안 된 추측 위에 쌓지 않는다. 실측(브라우저) -> 계약 -> 구현 순서. 계약과 실제가 어긋나면 [계약 실태 표](../../evolve-pyproc/references/contract-reality.md)에 먼저 기록한다.
5. **정공법.** 일부러 얇게 만들거나 우회하지 않는다. 막히면 우회 대신 `tests/attempts/` 또는 [계약 실태 표](../../evolve-pyproc/references/contract-reality.md)에 막힌 지점을 기록하고 정면으로 푼다.
