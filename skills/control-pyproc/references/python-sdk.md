...
python -m pip install \
  "https://github.com/eddmpython/pyproc/releases/download/v0.0.28/pyproc_control-0.0.28-py3-none-win_amd64.whl"
```

On Linux x64 install `pyproc_control-0.0.28-py3-none-manylinux_2_28_x86_64.whl` from the same release. The
...
```sh
npm install --save-exact pyproc@0.0.28
python -m pip install \
  "https://github.com/eddmpython/pyproc/releases/download/v0.0.28/pyproc_control-0.0.28-py3-none-any.whl"
```
...
operations. The registry persists state, not the client object. See [Execution Memory](../../use-pyproc-machine/references/execution-memory.md).
...
from a durable `sending` state never repeats the provider call. See [Rehearse-Commit](../../commit-pyproc-effects/references/rehearse-commit.md).
...
originating control process and its in-process Machine checkpoints. See [Transactional AppSpace](../../transact-pyproc-app-state/references/app-space.md).
...
Each requirement exposes `candidateEvidence` from the pre-projection candidate universe. Complete inventories
...
remain available in browser-disabled profiles. See [experience verification](../../verify-browser-experience/references/verification.md) for the
