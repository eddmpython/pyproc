...
import { operationAbortError, throwIfOperationAborted } from "@web-machine/core";
import { V86BlockBuffer } from "./v86BlockBuffer.js";
import {
  readV86FileSystemVolume,
  serializeV86FileSystemState,
  writeV86FileSystemVolume,
} from "./v86FileSystemVolume.js";
import { V86PacketPort } from "./v86PacketPort.js";
import { V86DisplayPort } from "./v86DisplayPort.js";
import { V86InputPort } from "./v86InputPort.js";
import { V86FramebufferPort } from "./v86FramebufferPort.js";
import { V86PointerPort } from "./v86PointerPort.js";
import { V86ClockPort } from "./v86ClockPort.js";
...
class V86GuestAdapter {
  ...
  async boot(context, manifest, control) {
    throwIfOperationAborted(control, `${context.machineId}: v86 boot`);
  ...
  async pause(control) {
    throwIfOperationAborted(control, "v86 pause");
  ...
  async resume(control) {
    throwIfOperationAborted(control, "v86 resume");
  ...
  async snapshot(control) {
    throwIfOperationAborted(control, "v86 snapshot");
    await this._blockBuffer?.drain();
    await this._packetPort?.drain();
    await this._displayPort?.drain();
    await this._framebufferPort?.drain();
    if (this._blockMode !== "filesystem") {
      const state = new Uint8Array(await this._requireEmulator().save_state());
      throwIfOperationAborted(control, "v86 snapshot", { outcomeUnknown: true });
      return state;
    }
  ...
  async restore(payload, context, manifest, control) {
    throwIfOperationAborted(control, `${context.machineId}: v86 restore`);
    this._context = context;
    this._manifest = manifest;
    if (!this._emulator) await this._createEmulator({ autostart: false, attachInteractiveInputs: false, control });
    await this._emulator.restore_state(toArrayBuffer(payload));
    throwIfOperationAborted(control, `${context.machineId}: v86 restore`, { outcomeUnknown: true });
    this._clockPort?.synchronizeWallClock();
    if (this._blockMode === "filesystem") {
      this._volumeStats = await readV86FileSystemVolume({
        device: this._blockDevice(),
  ...
  async shutdown(control) {
    ...
    this._serialPort.rejectAll(new Error("x86 adapter: shutdown"));
    if (this._emulator) {
      await this._inputPort?.drain();
      await this._pointerPort?.drain();
      this._inputPort?.detach();
  ...
  async request(message, control) {
    ...
    return this._serialPort.slice(from, end);
  }
  ...
  async _createEmulator({ autostart, attachInteractiveInputs, control }) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const onDownloadError = (event) => {
        finish(reject, new Error(`x86 adapter asset download 실패: ${event?.file_name || "unknown"}`));
      };
      const onAbort = () => finish(reject, operationAbortError(control, "v86 engine create", { outcomeUnknown: true }));
      const timer = setTimeout(() => finish(reject, new Error(`x86 adapter engine ready timeout: ${timeoutMs}ms`)), timeoutMs);
