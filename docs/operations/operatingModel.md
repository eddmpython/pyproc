...
| 층 | 위치 | 담는 것 | 판정 질문 |
|---|---|---|---|
| 강행규칙 | [CLAUDE.md](../../CLAUDE.md) | 위반 시 즉시 이력 오염·계약 파손이 나는 규칙만, 짧게. 각 규칙은 상세 문서를 가리킨다 | "어기면 즉시 손상인가?" |
| 공개 운영 문서 | [docs/](../) | 외부 기여자·소비자가 읽을 이유가 있는 객관 운영 규칙 | "외부인이 읽을 이유가 있는가?" |
| 로컬 메모리 | 저장소 밖(git 미추적) | 소유자와의 세션 간 약속·행동 규약. 인덱스 1파일 + 토픽 파일, 포인터 원칙(레포에서 도출 가능한 내용 복제 금지) | 위 둘 다 아니오일 때 |
