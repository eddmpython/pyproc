...
- 브레이킹(공개 표면·subpath export·타입 시그니처 변경)은 릴리즈 노트(커밋 메시지 본문)에 명시한다. 공개 시그니처는 [패키지 계약](../consuming/contract.md) 참조.
...
- 배포 설치는 **npm 정확 버전**을 핀한다: `"pyproc": "0.0.11"`(+ 락파일). 플로팅(`^`/`~`/`latest`) 금지. 재현성은 정확 버전 + 락파일이 보장하고, 릴리즈 없이 최신 커밋이 급하면 SHA 핀(`github:eddmpython/pyproc#<sha>`)이 대안이다.
...
- 기본 Pyodide는 `v314.0.2`(CPython 3.14). 스냅샷-fork가 Pyodide 밑줄(실험) API에 의존하므로 **Pyodide 버전 변경은 그 자체로 릴리즈 사유**이고, 패키지 계약과 브라우저 gate를 함께 재검증해야 한다.
