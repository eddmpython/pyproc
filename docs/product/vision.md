...
pyproc은 Web Machine Platform의 첫 Python guest OS다. 현재 배포되는 제품은 여전히 서버 없이 브라우저에서 Python 실행·프로세스·파일·권한·네트워크 가상화·복원 리액티브를 제공하는 재사용 커널이다. codaro/dartlab/xlpod는 이 검증된 공개 표면을 소비한다.

상위 목표가 커져도 현재형 주장은 넓히지 않는다. 범용 host, 공통 `.webmachine` 이미지, Linux Dual-Boot는 [web-machine-platform](../../mainPlan/web-machine-platform/README.md)의 미래 게이트이며 아직 pyproc 공개 API가 아니다.
...
- 현재형의 범용 Web Machine host 또는 x86 emulator. 이는 상위 North Star이며, 두 엔진과 Linux guest가 같은 계약을 통과하기 전에는 제품 주장으로 쓰지 않는다.
...
- 상위 Web Machine 비전과 Dual-Boot 게이트: [mainPlan/web-machine-platform](../../mainPlan/web-machine-platform/README.md)
- 첫 Python guest OS 성숙 트랙: [mainPlan/browser-os-north-star](../../mainPlan/browser-os-north-star/README.md)
