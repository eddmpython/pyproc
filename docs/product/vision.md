...
**브라우저에 OS를 만든다. 정확히는 Chromium 탭을 하드웨어/보안 경계로 삼고, 파이썬 런타임을 커널처럼 세워 프로세스·병렬·파일·권한·네트워크 가상화·복원 리액티브를 제공하는 Browser Python OS를 만든다. codaro/dartlab/xlpod는 이 OS 위에 자기 제품 표면을 얹고, pyproc은 공통 커널의 단일 진실(SSOT)이 된다.**

이 목표에서 물러서지 않는다. 다만 "OS"는 리눅스 복제품이라는 뜻이 아니라, 브라우저가 허용하는 하드웨어 경계 안에서 실행·상태·파일·프로세스·권한·네트워크를 일관된 시스템으로 중재한다는 뜻이다. 현재형 주장은 실측된 만큼만 하고, 나머지는 가상화·upstream 대기·웹 보안상 영구 벽으로 분류한다.
...
- **성공**: 소비 제품들이 pyproc을 실제 import해서 각자 표면을 얹고, 브라우저 파이썬 OS 개선이 pyproc 한 곳에 모인다. 소비자는 능력 계약만으로 복원 리액티브·프로세스 병렬·파일 세계·권한·가상 오리진을 쓰고 엔진 내부를 만지지 않는다. OS 간판은 별도 판정표에서 프로세스·메모리·파일·IPC·보호·부팅·영속·개발 표면을 실측으로 통과해야 한다.
