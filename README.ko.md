...
**North Star: 로컬에서 되는 모든 파이썬을 언젠가 브라우저에서도 돌린다, 서버 없이.** 이것은 현재 호환성 주장이 아니라 *방향*이다 - 목표는 일부러 무한대로 잡는다(snapshot-fork, 시간여행, 이동 가능한 머신 이미지 전부 그렇게 크게 잡아서 나온 발명이다). 현재형 주장은 브라우저가 실측한 만큼만. 로컬에서 되는 것은 네 가지 상태로 갈리고, pyproc의 일은 이것들을 위 칸으로 밀어 올리는 것과 플랫폼이 벽을 다시 여는 순간 가장 먼저 흡수하는 구조가 되는 것이다:

- **현재 달성**(오늘 브라우저 실측): 순수 파이썬·Pyodide 빌드 패키지, 멀티코어 프로세스, 체크포인트/시간여행, 커널 내 ASGI, 터미널, 영속 FS, 이동 가능한 머신 이미지.
- **우회 가능**(브라우저 방식으로 대체): TCP `listen()`은 ASGI 앱으로, `os.fork`는 워커 커널로, 아웃바운드 소켓은 릴레이로.
- **upstream 대기**(지금은 막혔으나 플랫폼 발전으로 다시 열림): 네이티브 C 확장 wheel(Emscripten 정적 빌드 / WebAssembly 컴포넌트 모델), GPU(WebGPU), 진짜 threading - 추적 중이고 엔진 seam이 가장 먼저 흡수할 준비.
- **웹 보안상 영구 벽**: 탭은 외부 릴레이나 에이전트 없이 임의 인바운드 연결을 받거나 임의 네이티브 바이너리를 실행할 수 없다.

원칙: 목표는 무한대로, 현재형 주장은 증명된 만큼만. 모든 능력은 브라우저 실측, 모든 벽은 이름을 붙인다. 축별 격차는 [local-parity](mainPlan/local-parity/README.md)가 추적한다.
