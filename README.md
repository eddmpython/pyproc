...
| `MachineJournal` | Write-ahead log: the machine checkpoints itself while idle, so a crashed tab still boots back into its last commit; `pack()` / `prune()` compact long-running OPFS blob stores |
