...
**살아 있는 이유**: 네 질문 중 일부만 승격됐다. 남은 질문의 실측 자리다.

runtimeParity(런타임 동작)와 별개의 개념 캠페인이다: **환경(패키지 세트)의 선언·캐시·재현.**
uv가 로컬에서 하는 것(즉시 부팅, 재현 가능한 락, 스크립트 자급)을 브라우저에서 성립시킨다.
현재 계약은 [패키지 계약](../../../skills/use-pyproc-runtime/references/consumer-contract.md)의 환경 관리 표면과 이 캠페인의 실측 표에 둔다.
...
(Cloudflare workerd 패턴의 브라우저판이며, 엔진 경계는 [EngineContract](../../../src/runtime/engineContract.js)가 정본이다).
