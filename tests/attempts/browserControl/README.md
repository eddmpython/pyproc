...
| 2026-07-13 | bootIsolationRunner(게이트8 관측) | Edge 150 headless | frame-busting sandbox 차단 시도가 COI offscreen에서 credentialless+sandbox 충돌로 iframe 로드 실패 | **iframe 역전의 부가 축(sandbox frame-busting 방어 + 쿠키 실림)은 non-COI 셸 필요 확증**(게이트4 쿠키 격리와 동일 뿌리). 로드만은 credentialless로 COI서도 가능. **셸/런타임 문서 분리가 iframe 역전의 전제** = 3중 확증(로드 트레이드오프/sandbox/쿠키) | non-COI 셸 필요 확정 |
| 2026-07-14 | bootIsolationRunner(게이트9-15) | Edge headless | Phase A 영속 세션(9/10) + Phase 2 non-COI 셸 쿠키·워커 N=세션 N·SW 재attach 복구·waitForSelector(11-14) + Phase 3 Playwright급 표면(15a-i: 추출/폼/포인터 isTrusted/신뢰 키보드/waitForFunction/screenshot PNG/에뮬레이션/히스토리/쿠키 왕복). **35/35 GREEN** | 영속 세션 모델 + 고정 화면 + 프로세스 OS 워커 라우터 + SW 소멸 복구 + **확장 조작 표면 전체**가 실측 성립. 상세 원장: [mainPlan/browser-control/03-progress-ledger](../../../mainPlan/browser-control/03-progress-ledger.md) | src 승격(protocol v2, 실 src 픽스처 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트16) | Edge headless | Phase 4 실전 배치(16a-g: 자동 스크롤 폴드 아래 클릭/다이얼로그 자동 처리 accept·reject·메시지/파일 업로드 setFileInputFiles/쿠키 삭제/네트워크 관측 waitForResponse/가로채기 route block·fulfill). **42/42 GREEN** | 다이얼로그·업로드·네트워크 가로채기까지 = 실제 사이트 자동화 핵심 격차가 좁아짐. 이벤트 소비는 driver당 단일 리스너로 카빙(증식 금지) | src 승격(실 src 픽스처 5/5 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트17) | Edge headless | Phase 5 네트워크 심화(17a-e: 요청 변조 route modify 헤더 주입/콜백형 held routing = 붙잡아 continue·fulfill·abort/응답 바디 캡처 responseBody). **47/47 GREEN** | 선언형 route를 넘어 **요청 단위 동적 제어**(붙잡아 Python이 결정)를 블로킹 모델 정합하게 실현. 경계: held는 비-항법 XHR(항법 하위요청 hold는 단일 스레드 교착) | src 승격(실 src 픽스처 5/5 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트18) | Edge headless | Phase 6 프레임 traversal(18a-c: frames 목록/프레임 내부 조회 text·exists·value·location/조작 fill·click DOM 변경, isolated world). **50/50 GREEN** | same-origin iframe 내부를 `createIsolatedWorld`로 드릴다운(고정 화면 셸 비전 직결). 경계: cross-origin은 OOPIF(별도 프로세스)라 메인 세션 frameTree 미포함 = setAutoAttach 필요한 별개 축(same-origin 전용) | src 승격(실 src 픽스처 5/5 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트19) | Edge headless | Phase 7 에뮬레이션 심화(19a-c: emulateMedia prefers-color-scheme dark/light/setTimezone Intl 관측/setOffline navigator.onLine). **53/53 GREEN** | 페이지 관측 환경 스푸핑(다크모드·타임존·오프라인)이 라이브로 성립. 정직한 컷: setGeolocation(grantPermissions가 tab-session에서 Browser 도메인 미접근)·setLocale(Edge CDP navigator.language 미반영)은 검증 불가라 미승격 | src 승격(실 src 픽스처 5/5 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트20) | Edge headless | Phase 8 파이썬 워커 N=세션 N 진짜 병렬: 워커 2개가 각자 Pyodide 부팅 + run_sync(JSPI) + offscreen 라우터(제약 A 우회)로 자기 세션 조작, CPU 연산(total=19999900000, 자기 GIL) + label 격리(workerA/workerB). **54/54 GREEN** | 프로세스 OS x 브라우저 컨트롤 융합을 **진짜 파이썬 인터프리터로** 실증(게이트12 plain JS 라우터 격상). 워커 N = 독립 GIL N = 세션 N. Playwright가 구조적으로 못 하는 축 | src 승격(routeBrowserWorker/installBrowserWorker, 실 src 픽스처 6/6 GREEN) |
...
| 2026-07-14 | bootIsolationRunner(게이트25) | Edge headless | Phase 13 지오로케이션 스푸핑(게이트19 벽 돌파): 좌표는 Emulation.setGeolocationOverride, 권한은 CDP Browser.grantPermissions(browser-level 막힘) 대신 chrome.contentSettings.location로 우회 부여. getCurrentPosition이 스푸핑 좌표(37.5665) 회수. **59/59 GREEN** | Phase 7의 "권한 부여 경로 없어 미승격"을 확장 API로 뚫음. 비용: manifest contentSettings 권한 | src 승격(실 src 픽스처 6/6 GREEN) |
| 2026-07-14 | bootIsolationRunner(게이트26) | Edge headless | Phase 14 로케일 스푸핑(Phase 7 두 번째 컷 되살림): setLocaleOverride 미반영이라 Accept-Language 헤더 + navigator.language/languages 선제 오버라이드로 대행. 항법 후 navigator.language=fr-FR + Accept-Language 반영. **60/60 GREEN** | Intl 기본 로케일은 미반영(정직) but navigator/Accept-Language로 사이트 로케일 감지엔 충분 | src 승격(실 src 픽스처 6/6 GREEN) |
