...
| 2026-07-13 | wasiUpgradeProbe | Edge headless, 로컬 COOP+COEP | **GREEN 9/9(해결).** brettcannon CPython 3.14.6 부팅 120ms, stdlib를 `/lib/python3.14` loose preopen(zlib 부재라 JS DecompressionStream 언집), 결정적 부팅, 체크포인트(40MB), **완전 시간여행**(변이+3MB 힙성장 -> 복원 후 재개 "100 False" -> 분기 "999"). | **3.12 동결 완전 해제 = WASI 레인이 3.14.6로.** 트랩 근원(연구 A): `os.read(0,1)`이 매 반복 새 bytes 할당 -> 그 포인터가 fd_read 경계를 넘어 삶(shadow stack + WASM VM 로컬 = 선형메모리 밖). 3.12는 obmalloc 우연으로 주소 안정(생존), 3.14는 주소 바뀌어 재개가 dereference하며 트랩. **해법 = 할당 불변 경계(`io.FileIO.readinto`로 고정 버퍼) + 파티션 복원**(스택 `[0,16MB)` 보존, 정적데이터+힙 `[16MB,end)` 복원). 레이아웃은 `--stack-first`(스택 [0,16MB], 데이터 [16-19MB], 힙 >19MB), stackTop=global[0] init 파싱. | src 승격 완료(wasiReplDriver readinto + wasiWorker 파티션). WLR 3.12 게이트도 GREEN 유지(엔진 무관 개선). 다음 = DEFAULT를 brettcannon로 이전 + engine-watch |
