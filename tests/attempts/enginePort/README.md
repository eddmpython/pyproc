...
|---|---|---|---|---|---|
| 2026-07-12 | wasiBootProbe | Edge headless, 로컬 COOP+COEP | GREEN 6/6. WLR CPython 3.12(WASI) 워커 부팅+print 253ms(Pyodide 콜드 ~3s 대비 빠름), exports.memory(heapU8 등가) 접근, json/sys 동작, **결정적 부팅 = 두 커널 파이썬 가시 상태 바이트 동일**(비결정 대조로 확인) | **"Pyodide를 뗀다"가 실측이 됐다.** 계약 코어 2축(선형 메모리 + 결정적 부팅)이 non-Pyodide에서 성립. WASI는 엔트로피가 import 2개로 수렴 = 더 깨끗한 결정성 | 값 프로토콜(FFI 우회 get/set), reactive를 이 엔진 위에서, 반복 실행(stdin 프레임 드라이버) |

## 판정

진행 중 (1번 관문 졸업: non-Pyodide 부팅 + 계약 코어 2축 GREEN. 남은 질문: 값 다리·반복 실행·reactive 이식.
승격 경로 = EngineContract에 WasiEngine 구현 추가 -> boot({engine})로 옵션화. 벽: 스택 sp 미노출(null 계약), 네이티브 확장 PEP 783 대기)
