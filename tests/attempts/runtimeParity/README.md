...
| Machine 안에 실제 source-pinned 명령을 상주시킬 수 있는가 | [ownedWasmToolLayerProbe.mjs](ownedWasmToolLayerProbe.mjs) | packed Machine의 VFS에서 실제 `rg`를 실행하고 exact argv, 입력 digest, exit, 출력, 상한, 취소, 무결성, 제3자 요청 0을 대조한다 |
...
| 실행 한 번이 print 출력과 마지막 식 값을 함께 주는가 | [cellValueProbe.mjs](cellValueProbe.mjs) | 12개 사례(식 꼬리, 문장 꼬리, None, 여러 줄 식, 깊은 괄호, 예외, 구문 오류) 통과 + 예외 뒤 재사용 -> `KernelSession.run` 계약으로 승격 |
...
| 2026-08-16 | ownedWasmToolLayerProbe | Windows 11, headed Edge 151.0.4129.86, exact packed Control | 첫 RED에서 `sys.platform=wasi`, 빈 `sys.executable`, `which(git|grep|rg)=None`, clone의 `subprocess.run(["rg","--version"])`은 `RuntimeError: memory access out of bounds`, Machine 도구 표면은 없었다. 첫 screenshot SHA-256은 `1d93dc98521f965721035bb7676eb470882fb0667a8243957ec391a90d9a94da`다. 공식 ripgrep 15.1.0 commit `af60c2de9d85e7f3d81c78601669468cf02dabab`의 크기 최적화 WASI binary는 2,193,626 bytes이고 실제 VFS 두 파일 검색 60ms, 반복 digest와 출력 일치, no-match exit 1, 출력 상한, 취소, 변조 거절, 제3자 요청 0이 GREEN이다. 최종 screenshot SHA-256은 `db74ac2ca3af74a363fb3c1a6ebc437dc2838069286e64397778d400bdb1b196`다. Gitoxide 0.54.0은 WASI에 없는 POSIX signal API로 compile 실패했다 | argv-only `machine.tools.run("rg", ...)`, read-only snapshot, 실행별 Worker, exact source와 lock, 내장 digest가 제품화됐다. Python `subprocess`와 Git은 지원하지 않는다 | 정식 `test:wasm-tools`로 승격했다. 다음은 WASI Git 구현과 같은 receipt의 Python bridge다 |
| 2026-08-16 | residentGitAndPythonToolBridgeProbe | Windows 11, headed Edge 151.0.4129.86, exact packed Control | 첫 RED는 Git `PYPROC_INPUT_INVALID`와 Python `ModuleNotFoundError`였고 screenshot SHA-256은 `ab1cb9d8db3dce59a30a43c92f1c180169d2eb0d7a3beebdee53c6fd3ac83ca1`다. libgit2 v1.9.7 exact commit `49e408b3208bc3093757a1c2db938d3590f3f412`와 repository patch를 WASI SDK 33, CMake 4.3.1, Ninja 1.13.2로 두 clean worktree에서 build해 1,636,225-byte SHA-256 `d346092d818597359d5bf1b78f52dfdc6e15e8f6ed18e70740e2087030f2d3b0` binary가 byte-identical이었다. 처음 불일치는 add의 stack trap, 그다음 object finalize 누락, config truncation이었고 각각 stack 상한, upstream lock rename, upstream file buffer 경로로 바로잡았다. 최종 headed gate는 rg, Git init, config, exact-path add, commit, log, clean status, local ref, main과 clone Python의 rg 및 동일 Git log receipt, 상한과 취소, 변조 거절, 제3자 요청 0을 10/10 통과했다. 직접 확인한 screenshot은 104,507 bytes, SHA-256 `2f6137c7d0354c7c0807347d2448195655f50a05b809ada60619330bbaf2f2c3`다 | `machine.tools.run("git", argv)`가 KernelVfs snapshot을 CAS fence로 확인한 뒤 bounded transaction을 반영하고, Python `pyprocTools`가 같은 catalog와 receipt를 호출한다. child kernelRef도 같은 layer에 붙고 close에서 분리된다. WASI Preview1 filestat timestamp offset은 40, 48, 56으로 계약화했다. network, shell, pipe, remote Git, 임의 Git CLI와 표준 라이브러리 `subprocess`는 지원 범위가 아니다 | 정식 `test:wasm-tools`, source contract, package provenance gate로 승격했다. 다음 자력 단이었던 `multiGuestComputer.nodeGuest`는 아래 Node 제품 증거로 완료됐다 |
| 2026-08-16 | nodeGuestProbe | Windows 11, Edge와 Chrome, exact packed product | 첫 RED는 기존 exact Buildroot Linux에서 `node --version`이 `node: not found`를 낸 결과다. Buildroot 2025.02.16과 Node 22.22.0을 고정한 독립 A/B build run `31930864003`에서 image, manifest, SBOM이 byte-identical이었다. 공개 자산 image는 31,048,192 bytes, SHA-256 `210fa467af2b718670e163f6802217086a9995661ac5a13e1529626a475568fc`다. Edge와 Chrome 제품 게이트가 Python, Linux, Node 공동 부팅, Node crypto와 filesystem, 6개 자산 영수증, 변조 선거부, signed image 상태 42 복원, 제3자 요청 0을 각각 8/8 통과했다. 직접 확인한 Edge screenshot은 111,439 bytes, SHA-256 `aa5d502758d35e5810fc017005b505e6f155ec0cc9d9fb266a93df6b05c0d359`, Chrome screenshot은 115,638 bytes, SHA-256 `62b1e6582d82a72fb169b3e700471e4703a47b68608f29e8c76b6fb6ff349650`다 | 실제 source-pinned Node guest가 common lifecycle, resource inspection, signed image, 공급망 검증 계약을 관통한다. package runtime dependency는 0이고 설치 제품이 제3자 요청 없이 부팅한다 | 정식 `test:node-guest`, asset catalog, source와 legal material, CycloneDX SBOM, 재현 manifest를 `buildroot-pyproc-node-i686-v1` release로 승격했다. 다음 천장 단은 `memory64`다 |

| 2026-09-25 | cellValueProbe | Windows 11, Edge 153 headless, exact packed product | 0.0.25 `run.python`은 `output`만 있고 값 필드가 없다. AST로 마지막 Expr만 떼어 eval하는 후보가 12/12 통과했고 깊이 90 괄호도 C 스택을 넘기지 않았다. 예외 뒤 같은 커널이 계속 42를 돌려준다. 호출 중앙값은 0.49ms에서 1.30ms(값 회수 왕복 포함) | 반복 루프 frame이 아니라 exec 안의 함수 frame에서는 명시 compile이 안전하다. REPL 의미론을 공개 실행 계약으로 올릴 수 있다 | 졸업 -> `KernelSession.run`(호출마다 넣는 helper, stderr 글은 output에 순서대로), Control 사설 하니스 삭제, `test:browser`와 `test:control-product` 수용 시험 |
## 판정
