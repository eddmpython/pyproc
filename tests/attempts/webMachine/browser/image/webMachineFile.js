...
async function payloadRecord(cryptoProvider, blobId, value) {
  const bytes = copyGenerationBytes(value, blobId);
  return {
    blob: {
      blobId,
      byteLength: bytes.byteLength,
      digest: await digestGenerationBytes(cryptoProvider, bytes),
    },
    bytes,
  };
}
...
export async function createWebMachineFile({
  ...
  const contentDigest = await digestGenerationManifest(cryptoProvider, content);
  const signature = await signWebMachineContent(cryptoProvider, contentDigest, signingKeyPair);
...
export async function readWebMachineFile({ file, cryptoProvider, trustedPublicKeys }) {
  assertFile(file);
  ...
  const manifestBytes = new Uint8Array(await file.slice(prefixByteLength, prefixByteLength + manifestByteLength).arrayBuffer());
  let manifestText;
  let parsed;
  try {
  ...
  const contentDigest = await digestGenerationManifest(cryptoProvider, getWebMachineManifestContent(manifest));
  ...
  const trust = await verifyWebMachineTrust(cryptoProvider, contentDigest, manifest.signature, trustedPublicKeys);
  ...
  for (const reference of manifest.blobs) {
    const bytes = new Uint8Array(await file.slice(offset, offset + reference.byteLength).arrayBuffer());
    const digest = await digestGenerationBytes(cryptoProvider, bytes);
