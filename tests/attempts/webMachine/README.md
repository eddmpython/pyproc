...
| 2026-07-15 | linuxGuestProbe 원격 image | Edge headless, COOP+COEP | **RED 0/1**. 원격 Buildroot image download-error | 격리 브라우저에서 외부 image 전달이 불안정했다. SHA-256 검증 로컬 실험 캐시로 변경 | `prepareV86Assets.mjs` |
...
- `webMachineDraft.js`: engine import가 없는 host lifecycle, device permission, snapshot envelope, fencing.
- `fakeGuestAdapter.js`: 상태 전이와 실패 의미론만 검증하는 deterministic guest.
- `guestAdaptersDraft.js`: 공개 root export만 소비하는 pyproc/WASI adapter.
- `v86GuestAdapterDraft.js`: x86 engine constructor를 외부에서 주입받는 Linux adapter.
- `hostContractProbe.html`: fake guest 공통 contract와 오류 경계.
- `dualEngineProbe.html`: 실제 두 엔진이 같은 contract를 통과하는지 검증.
- `linuxGuestProbe.html`: Buildroot Linux boot와 RAM state live/cold restore 검증.
- `dualBootProbe.html`: pyproc Python OS와 Linux를 한 registry에서 동시 boot/cold restore.
...
```bash
node tests/attempts/webMachine/prepareV86Assets.mjs
node tests/browser/run.mjs tests/attempts/webMachine/linuxGuestProbe.html
```

두 실제 엔진과 Linux guest 통과 조건은 충족했다. 그러나 common block/network/display와 영속 commit
경계가 남았으므로 이번 변경에서는 `src/` 또는 `index.js`로 승격하지 않는다. 다음 승격 결정은 host를
pyproc 내부 레이어로 둘지 독립 package로 둘지까지 포함한다.
