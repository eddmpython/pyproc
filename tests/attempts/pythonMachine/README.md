...
| OS가 탭 죽음에서 사나 (P2 kernelElection) | [kernelElectionProbe.html](kernelElectionProbe.html) | Web Locks 리더 정확히 하나 + 3탭 RPC 정합 + RPC 왕복 p50 < 3ms + 리더 iframe 제거 시 failover < 5s + 저널 resume 상태 생존 |
...
| 2026-07-15 | machineImageProbe(/home + signature) | Edge headless, 로컬 COOP+COEP | `.pymachine` home payload + WebCrypto signature GREEN 11/11. /home/web 파일 트리가 단일 파일에 포함되고, trusted public key로 `trust: true` 없이 openMachine 부활. 다른 공개키는 거부. 파일 15MB, export 75ms, open 1608ms | `.pymachine`이 힙 델타와 디스크 스냅샷뿐 아니라 출처 검증까지 가진 portable machine image가 됐다 | 승격 -> `Session.exportImage({ includeHome, signingKey })`, `createMachineKeyPair`, `exportMachinePublicKey`, `openMachine({ trustedPublicKeys })` |
...
진행 중. 결정성/리플레이/성장/이미지+/home/디스크/오프라인 2단/부트 자산 SRI v2/Service Worker 자산 봉인/공유 커널/호스팅 독립 2단/파일 세계/init/체크포인트 나무/forkLive/저널 WAL/저널 pack-prune/autoPack/IPC/컨테이너/커널 선출/잡 컨트롤/권한 감옥은 실측과 승격을 통과했다. 잔여 심화축은 공개키 배포/권한 UI, 제품 표면 배선 확장이다.
