...
| 장기 저널 파일 수를 줄일 수 있나 | [journalPackProbe.html](journalPackProbe.html) | HEAD/PREV live blob만 pack 파일 1개로 묶고 loose/stale 파일 제거 + pack-only recover + PREV fallback |
...
| 2026-07-15 | journalPackProbe | Edge headless | `MachineJournal.pack()` GREEN 7/7. 2세대 커밋 후 loose blob 223개 -> pack 파일 1개 + loose 0개, pack 1614ms. `prune()`이 stale loose 1개와 stale pack 1개 제거. pack-only HEAD recover(value=22)와 HEAD 파손 후 pack PREV fallback(value=11) 모두 정상 | 장기 OPFS 파일 수 축이 닫혔다. 기존 loose blob 포맷은 계속 읽고, 새 `PACKS.json` + `pack/*.bin` 계층은 recover 경로에 투명하게 붙는다 | 승격 -> `MachineJournal.pack()`/`prune()`. 다음은 자동 정책(언제 pack할지)을 소비 제품이 정하는 문제 |
...
진행 중. 결정성/리플레이/성장/이미지+/home/디스크/오프라인 2단/부트 자산 SRI v2/Service Worker 자산 봉인/공유 커널/호스팅 독립 2단/파일 세계/init/체크포인트 나무/forkLive/저널 WAL/저널 pack-prune/IPC/컨테이너/커널 선출/잡 컨트롤/권한 감옥은 실측과 승격을 통과했다. 잔여 심화축은 공개키 배포/권한 UI, 제품 표면 배선 확장이다.
