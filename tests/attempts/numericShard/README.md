...
| 2026-07-13 | matmulSurfaceProbe | Edge headless(자가 호스팅 경로, CDN 0) | 승격 계약 `PyProc.matmul(a, b, {parts})` 검증. **전체 결과 원소 == JS 참조**(비정사각 37x23x19 + 4로 안 나눠떨어지는 M의 잔여 행블록, maxErr 0.00), 차원 불일치 명시적 에러, parts:1 == 전 워커(maxErr 0.00), **공정 종단 배속 2.48배**(1024, parts:1 5366ms vs 4워커 2166ms = SAB 셋업·재구성·결과 조립 오버헤드 포함한 실사용 숫자). GREEN 5/5 | **Phase 1 src 승격 완료.** 공유 출력 SAB에 워커들이 자기 행블록을 assign(파이썬 버퍼 프로토콜)으로 쓰고 main이 조립. 종단 2.48배는 순수 3.67배보다 낮지만(조립 비용 포함) 정직한 사용자 배속 | 승격 -> `PyProc.matmul` + Matrix 타입 |
