...
[패키지 계약](../../skills/use-pyproc-runtime/references/consumer-contract.md)에 둔다.
...
`node tests/webMachine/fixtures/v86/prepareAssets.mjs --consumer nodeGuest`로 명시한다. 네이티브
Linux CPython 제품 gate는 `--consumer linuxPython`이다. 소비자 이름이 catalog에 없거나 선택되는
자산이 하나도 없으면 실패한다.
