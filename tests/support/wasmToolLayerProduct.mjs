...
let machine=null; let badMachine=null;
try {
  ...
  machine=await boot({deterministic:true,kernelVfs:vfs,assetIntegrity});
  ...
  let unsupported=null; try { await machine.tools.run("git",["status"]); } catch(error) { unsupported=failure(error); }
  ...
  const checks={
    contract:inspection.tools?.protocol==="pyproc.wasm-tool-layer" && inspection.tools.version===1
      && inspection.tools.execution==="isolated-worker" && inspection.tools.filesystem==="read-only-input-snapshot"
      && inspection.tools.network===false && inspection.tools.shellParsing===false
      && inspection.tools.commands?.length===1 && inspection.tools.commands[0].command==="rg"
      && inspection.tools.commands[0].version==="15.1.0" && inspection.tools.state==="verified",
    ...
    failures:unsupported?.code==="PYPROC_INPUT_INVALID" && outputLimit?.code==="PYPROC_WORKER_TASK_ERROR"
  ...
  const report={ok:Object.values(checks).every(Boolean),checks,inspection:inspection.tools,version,search:first,
  ...
  show("failures",report.negative,checks.failures&&checks.nonzeroIsResult); show("integrity",integrityFailure,checks.integrity);
  ...
  const status=document.getElementById("status"); status.textContent=report.ok
    ? "GREEN: source-pinned rg runs inside the Machine with bounded evidence" : "RED: product contract drifted";
  ...
  const report={ok:false,fatal:failure(error)}; const status=document.getElementById("status");
  status.textContent="RED: "+report.fatal.message; status.className="red";
  await fetch("/probeReport",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(report)});
} finally { if(badMachine) await badMachine.close(); if(machine) await machine.close(); }
</script></body></html>`;
}
...
try {
  ...
  await writeFile(join(publicDir, "wasmToolLayer.html"), page({ "pyproc": target("."), "pyproc/runtime": target("./runtime") }));
