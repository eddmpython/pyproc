...
지금 사는 것: WebGPU probe 7개와 소켓 probe 4개(워커 자산 2개 포함). 헤드리스 Edge는 저장소의 SwiftShader 스위치로도
...
소켓 probe는 밖으로 나가는 진짜 TCP를 요구한다. CI가 무는 것은 저장소 안 릴레이와 로컬 TCP
오리진으로 닫은 밀폐 레인(`npm run test:socket`)이고, 여기 있는 것들은 실제 외부 호스트를
치는 수동 실측이다.
