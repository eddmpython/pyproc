...
import { BENCH_ARTIFACT_SCHEMA_VERSION, S0_SCENARIO, S0C_SCENARIO, S1_SCENARIO, S1L_SCENARIO, S2_SCENARIO, normalizeBenchArtifact } from "./benchArtifacts.mjs";
...
if (![S0_SCENARIO, S0C_SCENARIO, S1_SCENARIO, S1L_SCENARIO, S2_SCENARIO].includes(scenario)) fail("--scenario는 S0, S0C, S1, S1L 또는 S2여야 한다");
...
const artifact = {
  ...
  scenario,
  candidate,
  name: scenario === S0_SCENARIO ? "python ready latency" : (scenario === S0C_SCENARIO ? "python cold ready latency" : (scenario === S1L_SCENARIO ? "single-kernel numpy matmul latency" : (scenario === S2_SCENARIO ? "process map" : "numpy sharded matmul"))),
  command,
  commit: gitCommit(),
