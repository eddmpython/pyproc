...
export const NORTH_STAR_AXES = Object.freeze([
  ...
  Object.freeze({
    ...
    en: Object.freeze({
      ...
      state: "The exact-installed Control, MCP, FrameSpace, and Python paths share Situation, paged semantic inventory, durable locators, proof-carrying actions, provider-neutral action convergence, and resource accounting. A product gate repeats observe, screenshot, confirmed click, artifact deletion, detach, and target close 20 times and returns every owned resource, process, and temporary profile to baseline. The installed Control gate captures a 32768 CSS-pixel page, then rejects dimension, area, and absolute clip extent overflow before screenshot dispatch with measured bounds and stable limits. Viewport scrolling remains an explicit effect because it can run observers. The packed convergence gate binds every proof-carrying action to at most two candidates, one reobservation, zero effect retries, and 30 seconds before the first effect. Same-document stale and navigation replacement each send one verified effect, transient occlusion waits then sends one, and ambiguous or persistently occluded targets send zero. FrameSpace proves the same receipt and records one unknown effect attempt without retry. A headed installed-package gate verifies registered compute and rendered-pixel results on a nonfallback hardware adapter through two request-scoped GPU hostcalls.",
    ...
    ko: Object.freeze({
      ...
      state: "정확히 설치한 Control, MCP, FrameSpace, Python 경로가 Situation, page 단위 의미 inventory, 내구 locator, 증거를 싣는 action, provider-neutral action 수렴과 자원 계수를 공유한다. 제품 gate는 관찰, screenshot, confirmed click, artifact 삭제, detach, target close를 20회 반복하고 소유 자원, process, 임시 profile을 모두 기준선으로 되돌린다. 설치 Control gate는 CSS 32768px 문서를 캡처하고 dimension, area, absolute clip extent 초과는 screenshot 전송 전에 측정 bounds와 안정 limit로 거부한다. viewport scroll은 observer를 실행할 수 있어서 명시적 effect로 남는다. 설치 tarball 수렴 gate는 proof-carrying action마다 후보 최대 2개, 재관찰 최대 1회, effect retry 0회, 첫 effect 전 30초를 고정한다. 같은 문서 stale과 navigation 교체는 각각 검증된 effect 1회, 일시 가림은 기다린 뒤 1회, ambiguous와 지속 가림은 0회다. FrameSpace도 같은 영수증을 내고 outcome unknown effect 1회를 재시도하지 않음을 증명한다. headed 설치 제품 gate는 nonfallback hardware adapter와 request-scoped GPU hostcall 2회로 등록된 compute와 rendered-pixel 결과를 검증한다.",
    ...
    evidence: Object.freeze([
      ...
      Object.freeze({ path: "tests/browser/controlProtocolProduct.mjs", lane: "test:control-product" }),
