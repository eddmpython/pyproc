...
function assertPath(path) {
  if (typeof path !== "string" || !path.startsWith("/") || path === "/" || path.includes("\\") || path.includes("\0")) {
    throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: malformed path ${path}`);
  }
  if (path.split("/").slice(1).some((part) => !part || part === "." || part === "..")) {
    throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: invalid path component ${path}`);
  }
}

function assertDevice(device) {
  if (!device || device.kind !== "block" || typeof device.read !== "function" || typeof device.write !== "function") {
    throw new TypeError("v86 9P volume: a block device is required");
  }
}
...
async function collectFileSystem(fileSystem) {
  ...
  const visit = async (directory) => {
    ...
    if (!Array.isArray(names)) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: directory read failed ${directory}`);
    for (const name of names.slice().sort()) {
      const path = joinPath(directory, name);
      assertPath(path);
      const inodeId = fileSystem.SearchPath(path).id;
      if (inodeId < 0) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: no such inode ${path}`);
      if (fileSystem.IsDirectory(inodeId)) {
        entries.push({ path, type: "dir" });
        await visit(path);
      } else {
        const inode = fileSystem.GetInode(inodeId);
        if ((inode.mode & 61440) !== 32768) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: inode is neither a file nor a directory ${path}`);
        const bytes = await fileSystem.read_file(path);
        if (!(bytes instanceof Uint8Array)) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: file read failed ${path}`);
        entries.push({ path, type: "file", offset: payloadBytes, size: bytes.byteLength });
        chunks.push(bytes);
        payloadBytes += bytes.byteLength;
      }
      if (entries.length > maxEntries) throw new WebMachineError("WEB_MACHINE_VOLUME_CAPACITY", "v86 9P volume: entry limit exceeded");
    }
  };
  await visit("/");
...
function validateMeta(meta, payloadBytes) {
  if (!meta || meta.version !== 1 || meta.payloadBytes !== payloadBytes || !Array.isArray(meta.entries) || meta.entries.length > maxEntries) {
    throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: metadata mismatch");
  }
  const seen = new Set();
  let nextOffset = 0;
  for (const entry of meta.entries) {
    assertPath(entry?.path);
    if (seen.has(entry.path)) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: duplicate path ${entry.path}`);
    seen.add(entry.path);
    if (entry.type === "dir") continue;
    if (entry.type !== "file" || !Number.isInteger(entry.offset) || !Number.isInteger(entry.size) || entry.offset !== nextOffset || entry.size < 0) {
      throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: file range is invalid ${entry.path}`);
    }
    nextOffset += entry.size;
    if (nextOffset > payloadBytes) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: payload is out of range");
  }
  if (nextOffset !== payloadBytes) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: payload size mismatch");
}
...
export function serializeV86FileSystemState(fileSystem) {
  ...
  if (!Array.isArray(state) || state.length !== 5 || !Array.isArray(state[0]) || !Array.isArray(state[2])) {
    throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: unsupported filesystem state");
  }
  return [
...
export async function writeV86FileSystemVolume({ device, fileSystem }) {
  ...
  if (frameLength > device.byteLength) throw new WebMachineError("WEB_MACHINE_VOLUME_CAPACITY", `v86 9P volume: capacity exceeded ${frameLength}/${device.byteLength}`);
...
export async function readV86FileSystemVolume({ device, fileSystem, emptyState, allowEmpty = false }) {
  ...
  if (prefix.every((byte) => byte === 0)) {
    ...
    throw new WebMachineError("WEB_MACHINE_VOLUME_EMPTY", "v86 9P volume: the block is uninitialized");
  }
  if (!magic.every((byte, index) => prefix[index] === byte)) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: magic mismatch");
  ...
  if (headLength <= 0 || headOffset + headLength > device.byteLength) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: head range is invalid");
  ...
  if (!Number.isInteger(meta?.payloadBytes) || meta.payloadBytes < 0 || payloadOffset + meta.payloadBytes > device.byteLength) {
    throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", "v86 9P volume: payload length is invalid");
  }
  ...
  for (const entry of directories) {
    ...
    if (parentId < 0) throw new WebMachineError("WEB_MACHINE_VOLUME_INVALID", `v86 9P volume: missing parent ${entry.path}`);
