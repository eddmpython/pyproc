...
async function collectFileSystem(fileSystem) {
  ...
  const visit = async (directory) => {
    ...
    if (!Array.isArray(names)) throw new Error(`v86 9P volume: directory read 실패 ${directory}`);
    for (const name of names.slice().sort()) {
      const path = joinPath(directory, name);
      assertPath(path);
      const inodeId = fileSystem.SearchPath(path).id;
      if (inodeId < 0) throw new Error(`v86 9P volume: inode 없음 ${path}`);
      if (fileSystem.IsDirectory(inodeId)) {
        entries.push({ path, type: "dir" });
        await visit(path);
      } else {
        const inode = fileSystem.GetInode(inodeId);
        if ((inode.mode & 61440) !== 32768) throw new Error(`v86 9P volume: file/dir 이외 inode ${path}`);
        const bytes = await fileSystem.read_file(path);
        if (!(bytes instanceof Uint8Array)) throw new Error(`v86 9P volume: file read 실패 ${path}`);
        entries.push({ path, type: "file", offset: payloadBytes, size: bytes.byteLength });
        chunks.push(bytes);
        payloadBytes += bytes.byteLength;
      }
      if (entries.length > maxEntries) throw new Error("v86 9P volume: 엔트리 상한 초과");
    }
  };
  await visit("/");
...
function validateMeta(meta, payloadBytes) {
  if (!meta || meta.version !== 1 || meta.payloadBytes !== payloadBytes || !Array.isArray(meta.entries) || meta.entries.length > maxEntries) {
    throw new Error("v86 9P volume: meta 불일치");
  }
  const seen = new Set();
  let nextOffset = 0;
  for (const entry of meta.entries) {
    assertPath(entry?.path);
    if (seen.has(entry.path)) throw new Error(`v86 9P volume: 중복 경로 ${entry.path}`);
    seen.add(entry.path);
    if (entry.type === "dir") continue;
    if (entry.type !== "file" || !Number.isInteger(entry.offset) || !Number.isInteger(entry.size) || entry.offset !== nextOffset || entry.size < 0) {
      throw new Error(`v86 9P volume: file 범위 위반 ${entry.path}`);
    }
    nextOffset += entry.size;
    if (nextOffset > payloadBytes) throw new Error("v86 9P volume: payload 범위 초과");
  }
  if (nextOffset !== payloadBytes) throw new Error("v86 9P volume: payload 크기 불일치");
}
...
export function serializeV86FileSystemState(fileSystem) {
  ...
  if (!Array.isArray(state) || state.length !== 5 || !Array.isArray(state[0]) || !Array.isArray(state[2])) {
    throw new Error("v86 9P volume: 지원하지 않는 filesystem state");
  }
  return [
...
export async function writeV86FileSystemVolume({ device, fileSystem }) {
  ...
  if (frameLength > device.byteLength) throw new Error(`v86 9P volume: 용량 초과 ${frameLength}/${device.byteLength}`);
...
export async function readV86FileSystemVolume({ device, fileSystem, emptyState, allowEmpty = false }) {
  ...
  if (prefix.every((byte) => byte === 0)) {
    ...
    throw new Error("v86 9P volume: 초기화되지 않은 block");
  }
  if (!magic.every((byte, index) => prefix[index] === byte)) throw new Error("v86 9P volume: magic 불일치");
  ...
  if (headLength <= 0 || headOffset + headLength > device.byteLength) throw new Error("v86 9P volume: head 범위 위반");
  ...
  if (!Number.isInteger(meta?.payloadBytes) || meta.payloadBytes < 0 || payloadOffset + meta.payloadBytes > device.byteLength) {
    throw new Error("v86 9P volume: payload 길이 위반");
  }
  ...
  for (const entry of directories) {
    ...
    if (parentId < 0) throw new Error(`v86 9P volume: parent 없음 ${entry.path}`);
