...
export function validateMachineHomeMeta(home, binLen) {
  ...
  for (const entry of home.entries) {
    if (typeof entry !== "object" || entry === null) throw new Error("machine: home 엔트리 형식 위반");
    validateRelativePath(entry.path);
    if (seen.has(entry.path)) throw new Error(`machine: home 엔트리 중복(${entry.path})`);
    seen.add(entry.path);
    if (entry.type === "dir") continue;
    if (entry.type !== "file") throw new Error(`machine: home 엔트리 타입 위반(${entry.type})`);
    if (!Number.isInteger(entry.offset) || !Number.isInteger(entry.size) || entry.offset !== nextOffset || entry.size < 0) throw new Error("machine: home 파일 오프셋 위반");
...
export function collectMachineHome(fs, path = DEFAULT_MACHINE_HOME_PATH, opts = {}) {
  ...
  if (!fs.exists(root)) {
    if (opts.required) throw new Error(`${opts.errorPrefix || "machineHome"}: ${root} 경로가 없어 /home 스냅샷을 만들 수 없다`);
    return null;
  }
  const rootStat = fs.stat(root);
  if (!rootStat.isDir) throw new Error(`${opts.errorPrefix || "machineHome"}: ${root}는 디렉터리가 아니다`);
  const entries = [];
  const chunks = [];
  let total = 0;
  const visit = (dir, rel) => {
    for (const name of fs.readdir(dir).slice().sort()) {
      ...
      if (st.isDir) {
        ...
        if (!(data instanceof Uint8Array)) throw new Error(`${opts.errorPrefix || "machineHome"}: ${childRel} 읽기 형식 위반`);
        if (total + data.length > HOME_MAX_BYTES) throw new Error(`${opts.errorPrefix || "machineHome"}: home 스냅샷이 상한을 넘었다`);
        entries.push({ path: childRel, type: "file", offset: total, size: data.length });
        chunks.push(data);
        total += data.length;
      } else {
        throw new Error(`${opts.errorPrefix || "machineHome"}: ${childRel}는 파일/디렉터리가 아니다`);
      }
      if (entries.length > HOME_MAX_ENTRIES) throw new Error(`${opts.errorPrefix || "machineHome"}: home 엔트리가 상한을 넘었다`);
    }
  };
  visit(root, "");
