// socketBridge.js - Layer 1 능력: 파이썬 socket을 진짜 TCP에 배선한다(아웃바운드).
...
// 스레드는 Atomics 불가라 JSPI = runAsync 경로에서 동작). requests/urllib3/http.client가 같은 socket
// API라 이 심을 그대로 쓴다. 실측: tests/attempts/socketBridge. 인바운드(공개 서버)는 물리 벽이다
// (역터널 릴레이 = 별도 조각). 릴레이 계약은 소비자 교체 가능(Wisp/websockify/자체).
//
...
export class SocketBridge {
  ...
  install() {
    ...
    const bridge = {
      ...
      close: (id) => { const st = socks.get(id); if (st) { if (st.ws) { try { st.ws.close(); } catch (e) {} } socks.delete(id); } },
    };
