...
export class MachineJournal {
  ...
  async commit() {
    ...
    try {
      ...
      for (const p of pages) {
        ...
        if (await this._hasBlob(blobDir, key, lookupCache)) {
          knownKeys.add(key);
  ...
  async _hasBlob(blobDir, key, cache) {
    try {
  ...
  async _autoPackAfterCommit(result) {
    ...
    const stats = await this._looseBlobStats();
