...
export class GpuCompute {
  ...
  static async create() {
    if (!(typeof navigator !== "undefined" && "gpu" in navigator)) {
      throw new PyProcError("PYPROC_GPU_UNAVAILABLE", "GpuCompute: WebGPU is unavailable (no navigator.gpu). Chromium or Edge is required.");
    }
    let adapter = await navigator.gpu.requestAdapter();
    if (!adapter) adapter = await navigator.gpu.requestAdapter({ forceFallbackAdapter: true });
    if (!adapter) {
      throw new PyProcError("PYPROC_GPU_UNAVAILABLE", "GpuCompute: no WebGPU adapter. A headless browser exposes none - use a windowed browser with a hardware GPU.");
    }
  ...
  array(data, rows, cols) {
    if (!(data instanceof Float32Array)) throw new PyProcError("PYPROC_INPUT_INVALID", "gpuArray: data must be a Float32Array (WGSL has no f64, f32 only).");
    if (data.length !== rows * cols) throw new PyProcError("PYPROC_INPUT_INVALID", `gpuArray: data length (${data.length}) does not match rows*cols (${rows * cols})`);
...
export class GpuArray {
  ...
  matmul(other) {
    if (!(other instanceof GpuArray)) throw new PyProcError("PYPROC_INPUT_INVALID", "GpuArray.matmul: the argument must be a GpuArray");
    if (this.cols !== other.rows) throw new PyProcError("PYPROC_INPUT_INVALID", `GpuArray.matmul: shape mismatch (${this.rows}x${this.cols}) @ (${other.rows}x${other.cols})`);
  ...
  map(expr) {
    if (typeof expr !== "string" || !expr.length) throw new PyProcError("PYPROC_INPUT_INVALID", "GpuArray.map: expr must be a WGSL expression string over x (the element). Example: \"max(x, 0.0)\"");
  ...
  binary(other, expr) {
    if (!(other instanceof GpuArray)) throw new PyProcError("PYPROC_INPUT_INVALID", "GpuArray.binary: the argument must be a GpuArray");
    if (this.rows !== other.rows || this.cols !== other.cols) throw new PyProcError("PYPROC_INPUT_INVALID", `GpuArray.binary: shape mismatch (${this.rows}x${this.cols}) vs (${other.rows}x${other.cols})`);
    if (typeof expr !== "string" || !expr.length) throw new PyProcError("PYPROC_INPUT_INVALID", "GpuArray.binary: expr must be a WGSL expression string over a and b (the two elements). Example: \"a + b\"");
    const device = this._gc._device, len = this.rows * this.cols;
    const cBuf = device.createBuffer({ size: len * 4, usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC });
    const nBuf = device.createBuffer({ size: 16, usage: GPUBufferUsage.UNIFORM, mappedAtCreation: true });
    new Uint32Array(nBuf.getMappedRange()).set([len, 0, 0, 0]); nBuf.unmap();
  ...
  async reduce(op) {
    if (!REDUCE_OPS[op]) throw new PyProcError("PYPROC_INPUT_INVALID", `GpuArray.reduce: op must be sum, max, or min (got: ${op})`);
